...
|---|---|
| `.github/` | Contains GitHub-related configuration files such as workflows, issue templates, or repository automation files. |
| `esp32-firmware/` | Contains the ESP32 firmware code responsible for reading sensor data and sending it to Firebase. |
| `esp32-firmware/main.cpp` | Main ESP32 firmware source file written using C++ / Arduino framework. |
| `esp32-firmware/secrets.h` | Stores Wi-Fi and Firebase credentials locally. This file should not be committed publicly. |
| `skycast-dashboard(Web-Dashboard)/` | Contains the React + Vite web dashboard for visualizing live environmental data. |
| `skycast-dashboard(Web-Dashboard)/src/` | Main source code folder for React components, hooks, Firebase configuration, charts, themes, and dashboard logic. |
| `skycast-dashboard(Web-Dashboard)/public/` | Contains static public assets used by the web dashboard. |
| `skycast-dashboard(Web-Dashboard)/dist/` | Production build output generated by Vite. |
| `skycast-dashboard(Web-Dashboard)/.firebase/` | Firebase Hosting cache/configuration folder generated during deployment. |
| `skycast-dashboard(Web-Dashboard)/.env` | Stores frontend environment variables such as Firebase config and Gemini API key placeholders. |
| `skycast-dashboard(Web-Dashboard)/firebase.json` | Firebase Hosting configuration file. |
| `skycast-dashboard(Web-Dashboard)/.firebaserc` | Firebase project alias/configuration file. |
| `skycast-dashboard(Web-Dashboard)/package.json` | Contains web dashboard dependencies and npm scripts. |
| `skycast-dashboard(Web-Dashboard)/vite.config.js` | Vite configuration file for the React dashboard. |
| `SkyCast(Android-App)/` | Contains the native Android application project. |
| `SkyCast(Android-App)/app/` | Main Android application module containing UI, activities, Firebase logic, and app resources. |
| `SkyCast(Android-App)/gradle/` | Contains Gradle wrapper-related project files. |
| `SkyCast(Android-App)/build.gradle.kts` | Root Gradle Kotlin DSL build configuration file. |
| `SkyCast(Android-App)/settings.gradle.kts` | Android project settings file. |
| `SkyCast(Android-App)/gradle.properties` | Gradle configuration properties. |
| `SkyCast(Android-App)/gradlew` | Gradle wrapper script for Linux/macOS. |
| `SkyCast(Android-App)/gradlew.bat` | Gradle wrapper script for Windows. |
| `SkyCast(Android-App)/local.properties` | Local Android SDK path configuration. This file should not be committed publicly. |
